  ...
  const onHandleSubmit = async (data: DataForm) => {
    try {
      ...
      if (file !== null) {
        ...
        formData.append("thumbnail", "https://localhost:3001/image.jpg");
      }
      formData.append("tagline", data.tagline);
      formData.append("categoryId", data.categoryId);
      formData.append("description", data.description);
      console.log(formData.values());
      await mutateAsync({ name: data.name, thumbnail: file || "https://localhost:3001/image.jpg", tagline: data.tagline, categoryId: data.categoryId, description: data.description });
      ...
      <form onSubmit={handleSubmit(onHandleSubmit)} className="flex flex-col w-[550px] rounded-[30px] p-[30px] gap-[30px] bg-[#F8FAFB]">
        ...
        <div className="relative flex flex-col gap-[10px]">
          ...
          <div id="thumbnail-preview-container" className="relative flex shrink-0 w-full h-[200px] rounded-[20px] border border-[#CFDBEF] overflow-hidden">
            <button onClick={() => inputFileRef.current?.click()} type="button" id="trigger-input" className="absolute top-0 left-0 w-full h-full flex justify-center items-center gap-3 z-0">
              {!file && (
          ...
          <input {...register("thumbnail")} ref={inputFileRef} onChange={handleFileChange} type="file" id="thumbnail" accept="image/*" className="absolute bottom-0 left-1/4 -z-10" required />
